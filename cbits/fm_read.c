...
int
load_index_mem (void ** index, uchar *compress, ulong size)
{
 ...
 */
static int
open_file (char * filename, uchar ** file, ulong * size)
{
	...
	*file = malloc ((*size) * sizeof (uchar));
	...
	ulong t =
		(ulong) fread (*file, sizeof (uchar), (size_t) * size,
			       outfile);
	if (t != *size)
		return FM_READERR;
 ...
 */
int
get_length(void *index, ulong *length)
{
